...
struct mat{
	...
	vec4 specular;
	vec4 emmission;		// x 
	vec4 smoothness;   // x - smoothness
	vec4 transparency; // x - isTransparent
						//y - ior
	vec4 volumetric; // x-density
					 // yzw - colour
};
...
void volumeSphere(Ray ray,inout RayHit bestHit, sphere sphere)
{
	vec3 oc = ray.origin - sphere.sphere_dat.xyz;
	float a = dot(ray.dir,ray.dir);
	float b = 2.0 * dot(oc,ray.dir);
	...
	float t0 = -b - sqrt(descrim);
	float t1 = -b + sqrt(descrim);
